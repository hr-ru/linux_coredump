...
Elf32_Xword	= ctypes.c_uint32	# typedef uint32_t Elf32_Xword;
...
EM_386		= 3			# #define EM_386       3 	              /* Intel 80386 architecture */
...
class Elf32_Ehdr(ctypes.Structure):			# typedef struct
	...
	("e_flags",	Elf32_Word),		#   Elf64_Word e_flags;
...
# Elf64/Elf32_Phdr related constants.
...
class Elf32_Phdr(ctypes.Structure):			# typedef struct
	...
	("p_filesz",	Elf32_Word),		#   Elf42_Word p_filesz;
	("p_memsz",	Elf32_Word),		#   Elf64_Word p_memsz;
...
class Elf32_Nhdr(ctypes.Structure):		# typedef struct
	_fields_ = [			# {
	("n_namesz",	Elf32_Word),	#   Elf32_Word n_namesz;                  /* Length of the note's name.  */
	("n_descsz",	Elf32_Word),	#   Elf32_Word n_descsz;                  /* Length of the note's descriptor.  */
	("n_type",	Elf32_Word),	#   Elf32_Word n_type;                    /* Type of the note.  */
	]				# } Elf32_Nhdr;
...
# Elf32_Shdr related constants.

class Elf32_Shdr(ctypes.Structure):		# typedef struct
	_fields_ = [			# {
	("sh_name",	Elf32_Word),	#   Elf32_Word    sh_name;                /* Section name (string tbl index) */
	("sh_type",	Elf32_Word),	#   Elf32_Word    sh_type;                /* Section type */
	("sh_flags",	Elf32_Word),	#   Elf32_Word   sh_flags;               /* Section flags */
	("sh_addr",	Elf32_Addr),	#   Elf32_Addr    sh_addr;                /* Section virtual addr at execution */
	("sh_offset",	Elf32_Off),	#   Elf32_Off     sh_offset;              /* Section file offset */
	("sh_size",	Elf32_Word),	#   Elf32_Word   sh_size;                /* Section size in bytes */
	("sh_link",	Elf32_Word),	#   Elf32_Word    sh_link;                /* Link to another section */
	("sh_info",	Elf32_Word),	#   Elf32_Word    sh_info;                /* Additional section information */
	("sh_addralign",Elf32_Word),	#   Elf32_Word   sh_addralign;           /* Section alignment */
	("sh_entsize",	Elf32_Word)	#   Elf32_Word   sh_entsize;             /* Entry size if section holds table */
	]				# } Elf32_Shdr;
...
class elf_siginfo32(ctypes.Structure):		# struct elf_siginfo
	_fields_ = [			#   {
	("si_signo",	ctypes.c_int32),		#     int si_signo;                       /* Signal number.  */
	("si_code",	ctypes.c_int32),		#     int si_code;                        /* Extra code.  */
	("si_errno",	ctypes.c_int32)		#     int si_errno;                       /* Errno.  */
	]				#   };
...
class timeval32(ctypes.Structure):		# struct timeval
	_fields_ = [			#   {
	("tv_sec",	ctypes.c_int32),	#     __time_t tv_sec;            /* Seconds.  */
	("tv_usec",	ctypes.c_int32)		#     __suseconds_t tv_usec;      /* Microseconds.  */
	]				#   };
...
class user_regs_struct32(ctypes.Structure):		# struct user_regs_struct
	_fields_ = [				# {
 	("ebx",		ctypes.c_uint32),		#   __extension__ unsigned  long int ebp;
	("ecx",		ctypes.c_uint32),		#   __extension__ unsigned  long int ecx;
	("edx",		ctypes.c_uint32),		#   __extension__ unsigned  long int edx;
	("esi",		ctypes.c_uint32),		#   __extension__ unsigned  long int esi;
	("edi",		ctypes.c_uint32),		#   __extension__ unsigned  long int edi;
	("ebp",		ctypes.c_uint32),		#   __extension__ unsigned  long int ebx;
	("eax",		ctypes.c_uint32),		#   __extension__ unsigned  long int eax;
	("ds",		ctypes.c_uint32),		#   __extension__ unsigned  long int ds;
	("es",		ctypes.c_uint32),		#   __extension__ unsigned  long int es;
	("fs",		ctypes.c_uint32),		#   __extension__ unsigned  long int fs;
	("gs",		ctypes.c_uint32),		#   __extension__ unsigned long int gs;
	("orig_rax",	ctypes.c_uint32),		#   __extension__ unsigned long long int orig_rax;
	("eip",		ctypes.c_uint32),		#   __extension__ unsigned  long int eip;
	("cs",		ctypes.c_uint32),		#   __extension__ unsigned  long int cs;
	("eflags",	ctypes.c_uint32),		#   __extension__ unsigned long long int eflags;
	("esp",		ctypes.c_uint32),		#   __extension__ unsigned  long int esp;
	("ss",		ctypes.c_uint32),		#   __extension__ unsigned  long int ss;
	]					# };
...
elf_gregset32_t = user_regs_struct32

class elf_prstatus(ctypes.Structure):		# struct elf_prstatus
	_fields_ = [			#   {
	("pr_info",	elf_siginfo32),	#     struct elf_siginfo pr_info;         /* Info associated with signal.  */
	("pr_cursig",	ctypes.c_int16),	#     short int pr_cursig;                /* Current signal.  */
...
class elf_prstatus32(ctypes.Structure):		# struct elf_prstatus
	_fields_ = [			#   {
	("pr_info",	elf_siginfo),	#     struct elf_siginfo pr_info;         /* Info associated with signal.  */
	("pr_cursig",	ctypes.c_int16),	#     short int pr_cursig;                /* Current signal.  */
	("pr_sigpend",	ctypes.c_int32),	#     unsigned long int pr_sigpend;       /* Set of pending signals.  */
	("pr_sighold",	ctypes.c_int32),	#     unsigned long int pr_sighold;       /* Set of held signals.  */
	("pr_pid",	ctypes.c_int32),		#     __pid_t pr_pid;
	("pr_ppid",	ctypes.c_int32),		#     __pid_t pr_ppid;
	("pr_pgrp",	ctypes.c_int32),		#     __pid_t pr_pgrp;
	("pr_sid",	ctypes.c_int32),		#     __pid_t pr_sid;
	("pr_utime",	timeval32),	#     struct timeval pr_utime;            /* User time.  */
	("pr_stime",	timeval32),	#     struct timeval pr_stime;            /* System time.  */
	("pr_cutime",	timeval32),	#     struct timeval pr_cutime;           /* Cumulative user time.  */
	("pr_cstime",	timeval32),	#     struct timeval pr_cstime;           /* Cumulative system time.  */
	("pr_reg",	elf_gregset32_t),	#     elf_gregset_t pr_reg;               /* GP registers.  */
	("pr_fpvalid",	ctypes.c_uint32),		#     int pr_fpvalid;                     /* True if math copro being used.  */
...
class elf_prpsinfo32(ctypes.Structure):			# struct elf_prpsinfo
	_fields_ = [				#   {
	("pr_state",	ctypes.c_byte),		#     char pr_state;                      /* Numeric process state.  */
	("pr_sname",	ctypes.c_char),		#     char pr_sname;                      /* Char for pr_state.  */
	("pr_zomb",	ctypes.c_byte),		#     char pr_zomb;                       /* Zombie.  */
	("pr_nice",	ctypes.c_byte),		#     char pr_nice;                       /* Nice val.  */
	("pr_flag",	ctypes.c_uint32),		#     unsigned long int pr_flag;          /* Flags.  */
						# #if __WORDSIZE == 32
						#     unsigned short int pr_uid;
						#     unsigned short int pr_gid;
						# #else
	("pr_uid",	ctypes.c_short),		#     unsigned int pr_uid;
	("pr_gid",	ctypes.c_short),		#     unsigned int pr_gid;
						# #endif
	("pr_pid",	ctypes.c_int32),			#     int pr_pid, pr_ppid, pr_pgrp, pr_sid;
...
_SI_MAX_SIZE	= 128
_SI_PAD_SIZE	= int(_SI_MAX_SIZE/ctypes.sizeof(ctypes.c_int)) - 4
